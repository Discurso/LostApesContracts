 ...
 */
abstract contract MiddleERC20 is Context, IERC20, IERC20Metadata {
    ...
    function _transfer(
        ...
        if (isFeeActive && !isTaxless[sender] && !isTaxless[recipient] && !inSwap) {
            transferAmount = collectFee(sender, amount, recipient == pair, sender != pair && recipient != pair);
        }
    ...
    function collectFee(
        ...
        if(otherFee != 0) 
        {
            ...
            if (_isExcluded[address(this)]) {
                _balances[address(this)] = _balances[address(this)].add(otherFee);
            }
...
contract MyERC20 is MiddleERC20, Ownable {
    address public minter = 0x0000000000000000000000000000000000000000;

    constructor() MiddleERC20("My Token", "TKN") {
    }
